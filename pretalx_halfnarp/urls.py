...
urlpatterns = [
    re_path(
        f"^(?P<event>{SLUG_REGEX})/p/halfnarp/$",
        frontend.FrontendView.as_view(),
        name="frontend",
    ),
    re_path(
        f"^(?P<event>{SLUG_REGEX})/p/halfnarp/my-preferences/$",
        frontend_preferences_api.FrontendMyPreferencesApi.as_view(),
        name="frontend_my_preferences_api",
    ),
    re_path(
        rf"^orga/event/(?P<event>{SLUG_REGEX})/p/halfnarp/$",
        organiser.OrganiserView.as_view(),
        name="organiser",
    ),
    re_path(
        rf"^orga/event/(?P<event>{SLUG_REGEX})/settings/p/halfnarp/$",
